...
const ColourProductsPage = () => {
    ...
    return (
        <Box sx={{ margin: "0 auto" ,minHeight:"800px" }}>
            <Box>
                <Breadcrumb pageTitle={colour.name} basePath={`/${slug}`} />
            </Box>
            <Box>
                ...
                <Box>
                    ...
                    >
                        {isExpanded ? colour.description || "" : `${(colour.description || "").slice(0, 100)}...`}
                    </Typography>
            ...
            {showProducts ? (
                <ProductListingPage
                    fetchDataFunction={(filters) => productService.getByColourSlug(slug, filters)}
                    basePath={`/renkler/${slug}`}
                    visibleFilters={{
                        price: true,
                        brand: true,
                        wallpaperType: true,
...
export default ColourProductsPage;